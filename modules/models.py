...
def PRNet_Model(in_size, channels, wd=0., name='PRNet_Model'):
    ...
    x = conv_f(filters=size, strides=1, activation=ReLU())(x)
    x = ResBlock(size * 2, strides=2, name='resblock_1')(x)
    x = ResBlock(size * 2, strides=1, name='resblock_2')(x)
    x = ResBlock(size * 4, strides=2, name='resblock_3')(x)
    x = ResBlock(size * 4, strides=1, name='resblock_4')(x)
    x = ResBlock(size * 8, strides=2, name='resblock_5')(x)
    x = ResBlock(size * 8, strides=1, name='resblock_6')(x)
    x = ResBlock(size * 16, strides=2, name='resblock_7')(x)
    x = ResBlock(size * 16, strides=1, name='resblock_8')(x)
    x = ResBlock(size * 32, strides=2, name='resblock_9')(x)
    x = ResBlock(size * 32, strides=1, name='resblock_10')(x)

    # decoder
    x = DeconvUnit(size * 32, strides=1, name='deconv_1')(x)
    x = DeconvUnit(size * 16, strides=2, name='deconv_2')(x)
    x = DeconvUnit(size * 16, strides=1, name='deconv_3')(x)
    x = DeconvUnit(size * 16, strides=1, name='deconv_4')(x)
    x = DeconvUnit(size * 8, strides=2, name='deconv_5')(x)
    x = DeconvUnit(size * 8, strides=1, name='deconv_6')(x)
    x = DeconvUnit(size * 8, strides=1, name='deconv_7')(x)
    x = DeconvUnit(size * 4, strides=2, name='deconv_8')(x)
    x = DeconvUnit(size * 4, strides=1, name='deconv_9')(x)
    x = DeconvUnit(size * 4, strides=1, name='deconv_10')(x)
    x = DeconvUnit(size * 2, strides=2, name='deconv_11')(x)
    x = DeconvUnit(size * 2, strides=1, name='deconv_12')(x)
    x = DeconvUnit(size, strides=2, name='deconv_13')(x)
    x = DeconvUnit(size, strides=1, name='deconv_14')(x)
    x = DeconvUnit(3, strides=1, name='deconv_15')(x)
    x = DeconvUnit(3, strides=1, name='deconv_16')(x)
    out = DeconvUnit(3, strides=1, activation=sigmoid, name='deconv_out')(x)
